...
BEGIN
  DECLARE salt CHAR(8);
  DECLARE concat VARCHAR(28);
  DECLARE pass_hash BINARY(64);
  SET salt = make_salt(8);
  SET concat = CONCAT(salt, password);
  SET pass_hash = SHA2(concat, 256);
  INSERT INTO user_info VALUES (new_username, salt, pass_hash);
  INSERT INTO adopter_info VALUES (new_username, password);
...
DELIMITER !
CREATE FUNCTION authenticate(usrname VARCHAR(20), password VARCHAR(20))
...
BEGIN
  ...
  IF NOT EXISTS(SELECT * FROM user_info WHERE username = usrname) THEN
    RETURN 0;
  END IF;

  SELECT salt, password_hash INTO slt, pass_hash
  FROM user_info WHERE username = usrname LIMIT 1;
  SET concat = CONCAT(slt, password);

  IF SHA2(concat, 256) = pass_hash THEN 
    RETURN 1;
  END IF;
...
CALL sp_add_user('Favour', 'favourokodogbe1');
CALL sp_add_user('Riti', 'ragrawal2');
