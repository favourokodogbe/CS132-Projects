(function() {
    ...
    function init() {
        getData();
        id("search-btn1").addEventListener("click", getGenFilter);
        id("search-btn2").addEventListener("click", getClothFilter);
     ...
     * This function fetches the data from 
     * data.json which is all the information 
     * about the clothes so it can get the gender
     * for the filter.
     */
    async function getGenFilter() {
        let url = "/data";
        try {
            let resp = await fetch(url);
            let response = checkStatus(resp);
            response = await resp.json();
            genderResults(response);
        } catch(err) {
            handleError(err);
        }
    }

    function genderResults(clothes) {
        id("review-view").classList.add("hidden");
        id("main-view").classList.add("hidden");
        id("review-page").classList.add("hidden");
        id("cart-view").classList.add("hidden");
     ...
     * This function fetches the data from 
     * data.json which is all the information 
     * about the clothes to filter the clothing
     * type.
     */
    async function getClothFilter() {
        let url = "/data";
        try {
            let resp = await fetch(url);
            let response = checkStatus(resp);
            response = await resp.json();
            clothResults(response);
        } catch(err) {
            handleError(err);
        }
    }
